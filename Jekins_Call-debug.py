...
#------------------------------------------------------------------------------------------------------------------------------                                                
def update_cfg():
    for field in ["UART1_COM","UART2_COM","UART3_COM","AUX_COM"]:
        try:
        ...
        for otestitem in td_tstsff:
            ...
            for i in range(1,6):
                if d_result[test_number]['result']['loop%s'%str(i)]['status'] in ['Passed','Failed']:
                    ...
                    otestitem.SetField('TC_ACTUAL_TESTER',d_result[test_number]['Feature_Owner'])#Tester
...
def get_TestNumber_From_QC(qc_path, qc_campaign, filter_text,
   ...
   try:
       ...
       print "1-Click       : %s" % one_click_filter
...
def download_Firmware(power_supply_addr):
    ...
    if os.path.isfile(os.environ['Firmware_Under_Tested']):
        try:
            ...
            t = threading.Timer(180.0, kill_SubProcess, args=(p,pids))
            t.start()
            print t
...
#-------------------------
   ...
   memo_purpose = (lambda x : "Official " if "Done" in x else "1-Click-Checking")(os.environ['QC_Filter'])                        
